<x-admin-layout>
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="font-display text-3xl text-white">Edit Hari Libur</h1>
        </div>
    </div>
    ...
    <div class="rounded-xl bg-neutral-900 border border-white/10 overflow-hidden p-6">
        <form action="{{ route('admin.hari-libur.update', $hariLibur) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', $hariLibur->tanggal->format('Y-m-d')) }}" required class="w-full bg-neutral-800 border border-white/10 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('tanggal')
            ...
            <div class="mb-6">
                ...
                <input type="text" name="keterangan" id="keterangan" value="{{ old('keterangan', $hariLibur->keterangan) }}" required class="w-full bg-neutral-800 border border-white/10 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('keterangan')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
