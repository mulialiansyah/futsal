...
@section('content')
    <a href="{{ route('customer.booking.index') }}" class="text-sm text-green-600 hover:underline mb-4 inline-block">← Kembali
        ke Booking</a>
        ...
        <div class="bg-white rounded-xl shadow p-6">
            ...
            <form method="POST" action="{{ route('customer.booking.store') }}" id="bookingForm">
                ...
                <div class="bg-green-50 border border-green-200 rounded-xl p-4 mb-6">
                    ...
                    <p class="text-gray-400 text-xs mt-2">* Harga final dihitung ulang otomatis oleh sistem saat booking
                        disimpan.</p>
                </div>
    ...
    <script>
        function hitungEstimasiHarga() {
            ...
            // Hitung durasi dalam menit supaya lebih robust (tidak hanya jam HH:00)
            const [hmMulai, mmMulai] = jamMulai.split(':').map(Number);
            const [hmSelesai, mmSelesai] = jamSelesai.split(':').map(Number);

            const menitMulai = hmMulai * 60 + mmMulai;
            const menitSelesai = hmSelesai * 60 + mmSelesai;
            ...
            // sistem server menggunakan diffInMinutes / 60 dan round, jadi untuk preview gunakan pendekatan durasiJam
            const durasiJam = durasiMenit / 60;
            const total = durasiJam * harga;
